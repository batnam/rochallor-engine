...
func escalateReview(ctx context.Context, job handler.JobContext) (handler.Result, error) {
	applicationID, ok := job.Variables["applicationId"].(string)

	slog.Info("#### escalateReview called", "applicationId", applicationID)
	if !ok || applicationID == "" {
