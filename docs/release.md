...
This repo is a monorepo with **seven independently releasable components**, each shipped to a different registry. To avoid forced version coupling, the CI publish workflows are split per component and triggered by **component-prefixed git tags**: pushing a tag publishes only that component's artifact.
...
The matching workflow fires automatically. Each workflow also creates a GitHub Release named `Engine v1.3.0` (etc.) with auto-generated release notes. Per-component releases are **not** marked as the repo's "latest" release — each component has its own release stream and GitHub's single "latest" pointer would be meaningless across seven streams.
...
There is no special "release everything" tag. If a change to `proto/` requires regenerating every SDK, tag each affected SDK (`sdk-go-v…`, `sdk-node-v…`, `sdk-python-v…`, `sdk-java-v…`) plus `engine-v…` and `modeller-v…` as needed.
